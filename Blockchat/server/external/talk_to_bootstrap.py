        ...
        if address != bootstrap_addr: 
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    success = False
                    print(f"Broadcast to node {node_id} failed with status code: {response.status_code}")
...
def talk_to_bootstrap():
    ...
    try:
        ...
        new_transaction = Transaction(bootstrap_addr, node_address, "coins", 1000, f"Welcome to Blockchat node {node_id}", None)
        my_state.blockchain.add_transaction(new_transaction)

        my_state.wallets[bootstrap_addr].amount -= 1000
        node_wallet = Wallet(current_app.config['node_count'],node_address,node_public_key, 1000)
        my_state.add_wallet(node_address,node_wallet)

        response_data = {
            "status": "success",
            "id": current_app.config['node_count']
        }
        ...
        if (node_count +1) == node_num:
            threading.Thread(target=broadcast_ips_ports_pks, args=(bootstrap_addr, my_state)).start()
